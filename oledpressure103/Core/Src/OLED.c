...
uint8_t OLED_Init(I2C_HandleTypeDef *i2c_handleTypeDef)
{
	...
	if (HAL_I2C_IsDeviceReady(hi2c, OLED_ADDRESS, 20, 400) != HAL_OK)
	{
		return 0;
	}
		...
		uint8_t width, uint8_t pos_x, uint8_t pos_y)
{
	...
	for (uint8_t current_y = 0; current_y < height; current_y++)
	{
		...
		{
			current_byte =
					progmem_bitmap[current_y * byte_width + current_x / 8];
	...
	for (uint8_t x = 0; x < symbol[0]; x++)
	{
		for (uint8_t y = 0; y < Font.height; y++)
		{
			uint8_t tmp = symbol[1 + y / 8
					+ x * (Font.numchars - 1) / Font.width] & (0x01 << (y % 8));
			tmp = tmp ? m1 : m2;
