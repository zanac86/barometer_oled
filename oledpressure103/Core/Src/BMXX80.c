...
int32_t BMP180_readRawPressure()
{
    ...
    //      Delay_us(5000);
        ...
        if (oversampling == BMP180_STANDARD)
            while ((BMP180_Read8(BMP180_CONTROL) & BMP180_SCO));
        //      Delay_us(8000);
        else
            if (oversampling == BMP180_HIGHRES)
                while ((BMP180_Read8(BMP180_CONTROL) & BMP180_SCO));
            //      Delay_us(14000);
            else
                while ((BMP180_Read8(BMP180_CONTROL) & BMP180_SCO));
		...
		uint8_t pressure_oversampling, uint8_t mode)
{
	...
	BMP280_Write8(BMP280_SOFTRESET, 0xB6);
...
float BMP280_ReadTemperature(void)
{
	if (_mode == BMP280_FORCEDMODE)
	{
		...
		if (mode == BMP280_FORCEDMODE)
		{
			...
			int32_t var1 = ((((adc_T >> 3) - ((int32_t) t1 << 1))) * ((int32_t) t2))
					>> 11;
...
{
#ifdef BMP280
	BMP280_Init(i2c_handler, BMP280_TEMPERATURE_16BIT, BMP280_STANDARD,
	BMP280_FORCEDMODE);
#endif
#ifdef BME280
	BME280_Init(i2c_handler, BME280_TEMPERATURE_16BIT, BME280_PRESSURE_STANDARD,
			BME280_HUMINIDITY_STANDARD, BME280_FORCEDMODE);
#endif
}
